...
export class PrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    // Synchronize SQLite schema on startup
    try {
    ...
    try {
      ...
      if (userCount === 0) {
        console.log('[PrismaService] No users found in database. Running auto-seed...');
        await this.autoSeed();
      } else {
        console.log('[PrismaService] Database already seeded.');
      }
    } catch (err) {
      console.error('[PrismaService] Auto-seed check failed:', err);
    }
  }
  ...
  private async autoSeed() {
    ...
    const branchProfsouz = await this.branch.create({
      data: { 
        name: 'Profsouz',
    ...
    const branchDushanbe = await this.branch.create({
      data: { 
        name: 'Dushanbe',
    ...
    // Manager
    const manager = await this.user.create({
      data: {
        phone: '992000003',
        passwordHash: commonPasswordHash,
        firstName: 'Dilshod',
    ...
    const courseNextJs = await this.course.create({
      data: {
    ...
    const courseReact = await this.course.create({
      data: {
    ...
    for (const s of students) {
      await this.groupStudent.create({
        data: {
          groupId: group.id,
          studentId: s.user.id,
          status: s.status,
    ...
    await this.schedule.createMany({
      data: [
    ...
    for (let w = 1; w <= 5; w++) {
      const week = await this.week.create({
      ...
      for (const d of dates) {
        ...
        for (const s of students) {
          ...
          await this.journalEntry.create({
            data: {
              lessonId: lesson.id,
              studentId: s.user.id,
              attended,
